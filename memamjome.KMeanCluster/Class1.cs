...
namespace memamjome.KMeanCluster
{
    public static class Clustring
    {
        public static int[] KMean(double[][] rawData, int numClusters, int maxCount)
        {
        ...
        public static void ShowClustering(double[][] rawData, int numClusters, int[] clustering, bool newLine)
        {
            ...
            {
                var numberItemsDisplayerPerCluster = 0;
                for (int i = 0; i < rawData.Length && numberItemsDisplayerPerCluster < 10; ++i) // each tuple
                {
                    if (clustering[i] == k) // curr tuple i belongs to curr cluster k.
                    {
                        ...
                        for (int j = 0; j < rawData[i].Length; ++j)
                        {
                            ...
                            numberItemsDisplayerPerCluster++;
                        }
                        Console.WriteLine("");
                    }
                }
        ...
        public static IEnumerable<Tuple<double, double>> DataPointsPerCluster(double[][] rawData, int clusterNumber, int[] clustering, int numberOfItemsPerCluster)
        {
            var numberItemsDisplayerPerCluster = 0;

                for (int i = 0; i < rawData.Length && numberItemsDisplayerPerCluster < numberOfItemsPerCluster; ++i) // each tuple
                {
                    if (clustering[i] == clusterNumber) // curr tuple i belongs to curr cluster k.
                    {
                        for (int j = 0; j < rawData[i].Length; ++j)
                        {
                            yield return new Tuple<double, double>(rawData[i][0], rawData[i][1]);
                            numberItemsDisplayerPerCluster++;
                        }
                    }
                }
